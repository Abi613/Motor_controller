...
{
    return (channel == ADC_CH_CURRENT) ? ADC_CURRENT_CHANNEL : ADC_VBUS_CHANNEL;
}
...
{
    float voltage_mv = ((float)ADC_GetRaw(ADC_CH_VBUS) / (float)ADC_RESOLUTION) * (float)ADC_VREF_MV;
